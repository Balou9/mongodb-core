...
import { Query} from "./../../../deno_lib/connection/commands.ts"
// const ReadPreference = require('../../../lib/topologies/read_preference');
import { ReadPreference} from "./../../../deno_lib/topologies/read_preference.ts"
import { Callback, noop} from "./../../../deno_lib/utils.ts"
import { MongoError} from "./../../../deno_lib/errors.ts"

/** Executes given command. */
export function executeCommand(configuration:{[key:string]: any}, db: string, cmd:{[key:string]: any}, options:{[key:string]: any}={}, callback: Callback=noop) :void{
  // Optional options
  ...
  // Alternative options
const host: string = options.host || configuration.host;
  const port: number = options.port || configuration.port;

  // Attempt to connect
  const pool: Pool = new Pool(null, {
  host, port
    // bson: new bson()
  });

  // Add event listeners
  pool.on('connect', (_pool: Pool): void => {
  const query: Query = new Query(/*new bson(),*/ `${db}.$cmd`, cmd, {
      numberToSkip: 0,
      numberToReturn: 1
    });
    ...
    _pool.write(
      ...
      (err?: MongoError, result?: {[key:string]: any}):void => {
        // if (err) {
...
//
// function locateAuthMethod(configuration, callback) {
